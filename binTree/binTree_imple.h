...
template <typename T> Rank BinTree<T>::updateHeight(BinNodePosi<T> p) {
  return p->height = 1 + std::max(stature(p->lc), stature(p->rc));
}

template <typename T> void BinTree<T>::updateHeightAbove(BinNodePosi<T> p) {
  while (p) {
    updateHeight(p);
    p = p->parent;
  }
...
template <typename T> BinNodePosi<T> BinTree<T>::insert(const T &e) {
  _size = 1;
...
template <typename T> Rank BinTree<T>::remove(BinNodePosi<T> n) {
  trace(n) = nullptr;
  updateHeightAbove(n);
  Rank r = removeAt(n);
  _size -= r;
...
template <typename T> static Rank removeAt(BinNodePosi<T> n) {
  if (!n)
    return 0;
  static int num = 0;
...
template <typename T> BinTree<T> *BinTree<T>::secede(BinNodePosi<T> n) {
  trace(n) = nullptr;
